...
훈련셋을 몇 번 반복해서 학습할 것인가를 정하는 것이 에포크(epochs)라고 했습니다. 얼마정보 반복 학습이 좋을 지를 정하기 위해서 검증셋을 사용할 수 있습니다. 초기에는 에포크가 증가될수록 검증셋의 평가도 높아집니다. 그러다가 에포크가 어느 이상 증가되면, 더 이상 검증셋의 평가는 높아지지 않고 오버피팅이 되어 오히려 떨어집니다. 이 시점에서 학습을 그만두면 됩니다. 아래 그래프에서 세로축이 100문항 중 틀린 개수이고, 가로축이 모의고사 풀이 반복횟수입니다. 앞서 설명했듯이 풀이를 반복할수록 훈련셋(모의고사 1회~4회)에서는 틀린 개수가 적어짐을 보실 수 있습니다. 각 에포크에서 학습된 모델을 검증셋으로 평가를 했을 때도 처음에는 틀린 개수가 줄어들지만 어느 시점에서는 다시 증가됨을 보실 수 있습니다. 이 그래프에서는 11번 반복했을 때 이 현상이 나타났습니다. 이 시점이 적정 반복 횟수로 보고 학습을 중단합니다.
