...
배치사이즈는 몇 문항을 풀고 해답을 맞추는 지를 의미합니다. 100문항일 때, 배치사이즈가 100이면 전체를 다 풀고 난 뒤에 해답을 맞춰보는 것입니다. 우리가 해답을 맞춰볼 때 '아하, 이렇게 푸는구나'라고 느끼면서 학습하는 것처럼 모델도 이러한 과정을 통해 가중치가 갱신됩니다. 
    ...
    문제를 푼 뒤 해답과 맞춰봐야 학습이 일어난다. > 모델의 결과값과 주어진 라벨 값과의 오차를 줄이기 위해, `역전파(Backpropagation)` 알고리즘으로 가중치가 갱신된다.
...
실제로 얼마전 민방위 문제를 하나 틀렸는데, 해답을 보고나서 어의가 없었습니다. '대피한다'를 문구를 보고 무조건 답이 아니다라는 생각에 문제조차 제대로 읽지 않았습니다. 지금까지의 민방위 문제에 너무 오버피팅이 되었나 봅니다.

![plot](http://tykimos.github.com/Keras/warehouse/2017-3-25-Dataset_and_Fit_Talk_overfitting0.png)

(혹자는 '마음의 소리로 찍었다'라던지 '해안에서 지진을 느꼈다면, 대피가 무슨 소용이 있겠느냐'라고 위로해줬습니다.)
...
정확한 비유는 아니겠지만, 대학해서도 학점이 좋으면 조기졸업을 할 수 있습니다. 이 의미를 살펴보면, 학점이라는 것은 대학 자체 시험을 통해 검증을 해본 것이고, 더 이상 학기를 다녀봤자 학습이 일어나지 않을 것으로 판단해서 졸업을 시키는 것입니다. 실제로도 학점을 채우고 졸업 성적이 되면, 취업준비를 하거나 놀거나 해서 대학에서의 학습은 되지 않습니다. 학습도 되지 않은 학기를 비싼 등록금을 내면서 다닐 필요는 없는 것이죠. 딥러닝에서 비싼 등록금이란 어렵게 구축한 GPU 머신, training curve을 하염없이 지켜보고 있는 인력 등등이 있을 수 있겠습니다.
